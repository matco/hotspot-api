...
public class ConnectionProviderDb implements ConnectionProvider {
	...
	public ConnectionProviderDb() {
		...
		ds.setJdbcUrl(String.format("jdbc:mysql://%s:%s/%s?characterEncoding=utf-8&useUnicode=true&useLegacyDatetimeCode=false&serverTimezone=GMT&useSSL=false", properties.getProperty("db.host"), properties.getProperty("db.port"), properties.getProperty("db.name")));
